...
Scenario: Outbound FGND Flow Boston 
	Given Login as WMUser
	Then Create DO xml "FGND_Boston"
    Then Open "Post Message" screen under "Integration"
	Then Post PO xml "FGND_Boston"   
    Then Validate status for "Distribution Orders" - "Released"
    Then Open "Run Waves" screen under "Distribution"    
    Then Select Wave "Automation - BK1 - Non Ship Ready" and Run
    Then Select "Bulk Non Ship Ready" Define and Submit for "FGND_Boston"
    ...
    Then Input and Apply WaveNumber for "FGND_Boston"
    Then Validate EPI status for "FGND_Boston"
    Then Get details "ItemName" from xml for "FGND_Boston"
