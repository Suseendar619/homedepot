...
Scenario: LTL Outbound Flow Atlanta
    Then Login as WMUser
    Then Create DO xml "LTLOutboundAtlanta"
    Then Open "Post Message" screen under "Integration"
    Then Post PO xml "LTLOutboundAtlanta"
    Then Validate status for "Distribution Orders" - "Released"
    Then Open "Run Waves" screen under "Distribution"
    Then Select Wave "Automation - BK1 - Non Ship Ready" and Run
    Then Select "Bulk Non Ship Ready" Define and Submit for "LTLOutboundAtlanta"
    Then Open "Waves" screen under "Distribution"
    Then Input WaveNumber and Apply 
    Then Validate Wave Completion
    Then Validate status for "Distribution Orders" - "DC Allocated"
    Then Validate status for "oLPNs" - "Printed"
    Then Get details "ItemName" from xml for "LTLOutboundAtlanta"
	...
	Then Generate Shipment for "LTL_Baltimore"
	Then Open "Schedule Appointment" screen under "Distribution"
	Then Schedule Appointment "LTLOutboundBaltimore"
	Then Open "Check-In" screen under "Distribution"
	Then IB CheckIN "LTLOutboundDallas"
